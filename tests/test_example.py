...
class TestExample(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.area_name = "/Users/felix/Github/shapley/texas7k_2020_gen_to_zone.csv"
    ...
    def test_gen_cohort(self):
        ...
        result_df = gen_cohorts(self.num_scen,self.num_cohorts, self.folder_path, self.area_name) #Imagine my_result is generated by my code
        expected_result = pd.read_csv("tests/test_gen_cohort.csv", header = [0], index_col = [0,1]) #This is the result I expect to get for this test
    ...
    def test_shap_alloc(self):
        ...
        result_df = shap_alloc(self.num_scen, self.alpha, self.num_cohorts, 
                               pd.read_csv("tests/test_gen_cohort.csv", header = [0], index_col = [0,1])) #Imagine my_result is generated by my code
        expected_result = pd.read_csv("tests/test_shap_alloc.csv", header = [0], index_col = [0])
        pd.testing.assert_frame_equal(result_df, expected_result)
    
    '''    def test_gen_cohort_fails(self):
        #Example to what it looks like when a test fails
        result_df = gen_cohorts(self.num_scen,0, self.folder_path, self.area_name) #Imagine my_result is generated by my code
        self.assertRaises(defineException,result_df) #"This test will always fail."
    '''
