...
def gen_cohort(num_scen: int, num_cohorts: int, folder_path: str, area_fname: str):
    ...
    #Create the cohorts
    cohort = divide_chunks(gen_zone_df, cohort_size)
    del gen_zone_df
    #Sort the dataframe by area
    full_em_df.sort_values(by = "Area", axis = 1,inplace= True)
...
#################################
def shap_alloc(num_scen: int, alpha: float, num_cohort: int, scen_df: object):
    ...
    temp_mat = np.array([shap_alloc_helper(i+1, num_cohort) for i in range(num_cohort)])
    #Compute the shapley allocations
    scen_df = pd.concat([pd.DataFrame(data = np.matmul(scen_df.values, 
                                            np.transpose(temp_mat)), 
                           columns = ["aloc" + str(i +1) for i in range(num_cohort)], 
                           index = scen_df.index),
                         scen_df], axis =1 )

    #Compute the 95th percentile for each hour accross all scenarious in the BAU case
    scen_df["VaR"] = scen_df[char_order(num_cohort)[0]].groupby(level = "Hour").transform("quantile",
                                                                                 quantile)

    #Create a column that indicates if in the tail
    scen_df["inTail"] = scen_df[char_order(num_cohort)[0]] >= scen_df["VaR"]

    result_df = round(scen_df.loc[scen_df["inTail"], ["aloc" + str(i+1) for i in range(num_cohort)] + [char_order(num_cohort)[0]]]\
